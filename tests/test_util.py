...
class UtilTest(TestCase):
    ...
    def test_X(self):
        self.assertEqual(force_unicode(X(status=0)),
                         "(AND: ('status', 0))")
        self.assertEqual(force_unicode(X(status=0) & X(company_status__in=[0,6])),
                         "(AND: ('status', 0), ('company_status__in', [0, 6]))")
        self.assertEqual(force_unicode(X(status=0) | X(company_status=0)),
                         "(OR: ('status', 0), ('company_status', 0))")
        self.assertEqual(force_unicode(X(with_photo=True)),
                         "(AND: ('with_photo', True))")
        self.assertEqual(force_unicode(X(date_created__gt=datetime(2012, 5, 17, 14, 35, 41, 794880))),
                         "(AND: ('date_created__gt', datetime.datetime(2012, 5, 17, 14, 35, 41, 794880)))")
        self.assertEqual(force_unicode(X(price__lt=1000)),
                         "(AND: ('price__lt', 1000))")
        self.assertEqual(force_unicode(X(price__gte=100) & X(price__lte=1000)),
                         "(AND: ('price__gte', 100), ('price__lte', 1000))")
        self.assertEqual(force_unicode(X(price__between=[500, 1000])),
                         "(AND: ('price__between', [500, 1000]))")
        self.assertEqual(force_unicode(X(price__range=[2, 10])),
                         "(AND: ('price__range', [2, 10]))")
        self.assertEqual(force_unicode(X(category__in=[1, 2, 3, 4, 5]) & (X(status=0) | X(status=5) | X(status=1) & X(company_status=6))),
                         "(AND: ('category__in', [1, 2, 3, 4, 5]), (OR: ('status', 0), ('status', 5), (AND: ('status', 1), ('company_status', 6))))")
        self.assertEqual(force_unicode(~X(status=1)),
                         "(AND: (NOT (AND: ('status', 1))))")
        self.assertEqual(force_unicode(~X(status__in=[1, 2, 3])),
